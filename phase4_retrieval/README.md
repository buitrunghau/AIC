...
3. **Cấu hình API Key (BẮT BUỘC):** 
   Hệ thống sử dụng LLM qua SDK mới của Google (`google-genai`) để phân tách sự kiện. Bạn không được hardcode API key vào code.
   Hãy liên hệ với **Member 4 (AI & Retrieval Scientist)** để lấy `GEMINI_API_KEY`. Sau đó cấu hình biến môi trường trước khi chạy:
...
4. Lệnh import lazy: Hệ thống đang sử dụng APIEventSegmenter (dùng model gemini-3.7-flash) được nhúng trực tiếp trong `search.py` để tách câu. Bạn chưa cần cài đặt các thư viện Deep Learning phức tạp như `torch` hay mô hình ngôn ngữ lớn (BERT) tại môi trường local.
...
Test suite phủ sóng toàn bộ các kịch bản của DANTE (chọn sequence đúng, loại nhiễu, thứ tự tăng dần) và WRRF (hợp nhất chuẩn, giới hạn top_k).
...
Hiện tại, `search.py` đang import `search_milvus_db` và `search_elastic_db` từ `mock_database.py`. 
Khi Phase 3 đã hoàn thiện, bạn cần vào `phase4_retrieval/search.py` sửa lại import để kết nối với DB thật:
