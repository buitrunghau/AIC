...
9. [Giao diện tích hợp giữa các Module](#9-giao-diện-tích-hợp-giữa-các-module)
10. [Thiết kế phi chặn](#10-thiết-kế-phi-chặn)
...
## 3. Phase 1 – Data Pipeline Module
...
- **Adaptive Filtering:** Thay vì lấy khung hình cố định theo giây, hệ thống tính toán khoảng cách chuẩn \( L_2 \) giữa khung hình hiện tại \( e_{current} \) và khung hình trước đó \( e_{prev} \). Chỉ giữ lại khung hình nếu:
  \[
...
## 6. Phase 4 – Retrieval & Alignment Module
...
|------------|---------------------|--------------------|
| Cắt khung hình | TransNetV2 + DCNN cells | Mặc dù tính toán nặng hơn việc lấy mẫu tĩnh mỗi 1 giây, nhưng đảm bảo không mất mát các sự kiện lướt qua nhanh. |
| Backbone Vision | SigLIP2 & BEiT-3 | SigLIP giải quyết bài toán hàm suy hao softmax của CLIP bị thắt nút cổ chai (bottleneck), giúp vector hội tụ tốt hơn. |
| Database | Tách Milvus & Elasticsearch | ElasticSearch tìm từ khóa BM25 cực mạnh nhưng yếu toán học vector; Milvus xử lý HNSW siêu tốc. Việc tách biệt gây khó trong đồng bộ hóa `keyframe_id`, nhưng tối ưu hóa tốc độ độ trễ truy vấn. |
| Temporal Alignment | DANTE (Quy hoạch động) | Các phương pháp sinh tổ hợp (Brute-force) sẽ sập hệ thống khi \( N \) lớn. DP đảm bảo độ phức tạp đa thức và thời gian thực thi có thể kiểm soát. |
