"""
Phase 1: TransNetV2 Shot Boundary Detection
Loads pretrained model and detects scene cuts (hard cuts + dissolves).
"""
...
class ShotBoundaryDetector:
    """Wrapper for TransNetV2 shot boundary detection."""
    ...
    def __init__(self, model_path: Optional[str] = None, device: str = "cpu"):
        ...
        Args:
            model_path: Path to TransNetV2 pretrained weights
            device: 'cpu' or 'cuda'
        """
        self.device = device
        self.model = TransNetV2().to(device).eval()
        
        if model_path is None:
            model_path = Path(__file__).parent / "Model" / "transnetv2-pytorch-weights.pth"
